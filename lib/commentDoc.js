...
function parseXmlDocBlock(code) {
    ...
    var r = parser_1.ParseRegex(code, compose_1.cap(summaryBlockPatt), function (match) {
        return ({
...
function parseAttribute(code) {
    ...
    return parser_1.ParseRegex(code, attrib, function (match) {
        return ({
            name: match[1],
            value: match[2]
        });
...
var _a = (function () {
    ...
    var parseNode = function (type, nodeEnd) {
        return function (code) {
            return parser_1.ParseRegex(code, nodeStart(nodeEnd), function (match) {
                return ({
                    space: match[1],
                    tag: match[2],
...
function parseNodeEnd(code) {
    ...
    return parser_1.ParseRegex(code, nodeEnd, function (match) {
        return ({
            space: match[1],
            tag: match[2],
            type: "end"
...
function parseEmptyNode(code) {
    ...
    return parser_1.ParseRegex(code, patt, function (match) {
        return ({
            type: "emptyNode",
            space: match[1],
            tag: match[2]
